<?php
    ...
    if(isset($_POST["user"]) && isset($_POST["pregunta"]) )//si las variables existen
    {
        $user=mysqli_real_escape_string($connect,$_POST["user"]);//por si hay inyecsion sql
        $pass=mysqli_real_escape_string($connect,$_POST["pregunta"]);
        $sql="SELECT user FROM usuario WHERE (user ='$user' OR correo='$user') AND pregunta='$pregunta'";
